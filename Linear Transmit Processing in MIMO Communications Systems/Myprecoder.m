...
%         sym_noise = awgn(sym_HP,SNRdB(i));
        sym_noise = sym_HP + sqrt(1/snr)*(randn(size(sym_HP))+1i*randn(size(sym_HP)));
        
        % MF ZF WF
        Gmf = RxMF(P_orig,H,Rs,Rn,1);
        Gzf = RxWODE(H,P_orig,Rs,Rn);
        ...
        % Demodulation
        sym_demod_rxmf = pskdemod(sym_GHF_rxmf,4);
        sym_demod_rxzf = pskdemod(sym_GHF_rxzf,4);
        sym_demod_rxwf = pskdemod(sym_GHF_rxwf,4);
        ...
        % Receive signals
        sym_receive_txmf = H*Pmf*sym_trans + sqrt(1/snr)*(randn(size(sym_HP))+1i*randn(size(sym_HP)));
        sym_receive_txzf = H*Pzf*sym_trans + sqrt(1/snr)*(randn(size(sym_HP))+1i*randn(size(sym_HP)));
        sym_receive_txwf = H*Pwf*sym_trans + sqrt(1/snr)*(randn(size(sym_HP))+1i*randn(size(sym_HP)));
...
semilogy(SNRdB,BERzf(1,:),'r-+');
hold on;
semilogy(SNRdB,BERwf(1,:),'g-o');
