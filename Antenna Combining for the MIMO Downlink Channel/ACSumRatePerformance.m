...
% Antenna combining for the MIMO downlink siganl
clc
clear
% close
...
for idx1 = 1:1:length(FB)
    for idx2 = 1:1:length(snr)
        ...
        for idx3 = 1:1:hnum
            ...
            [F,s_proj,gamma] = Algorithm_antenna_combining(H,C); % good
%             [F,s_proj,gamma] = ACalgorithm(H,C); % bad
%             ZFpre = ACZeroForcing(F);
            ZFpre = RegZFprecoder(F,pow);
            Heff = effective_channel(H,gamma);
