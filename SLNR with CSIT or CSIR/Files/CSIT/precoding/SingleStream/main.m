% Simulation of precoding schemes for single stream per user
...
% Parameters:
% N: amount of transmitting antennas per BS
% K_all: amount of users waiting for being scheduled
% K: amount of the scheduled users
% Mi: amount of receiving antennas per user
% S: amount of data streams per user
...
for i1=1:K_all_Num,
    for i2=1:SNR_Num,
        ...
        for i3=1:LoopNum,
            ...
            p1 = ones(1,K*S);
            [count_temp,capacity_temp] = coherence_receive(N,Mi,S,K,KK_NoScheduling,Tc,s,x,H,sigma2,w,u1,p1);
            count1=count1+count_temp;
            capacity1=capacity1+real(capacity_temp);
            
            % regularizing the inversion, MMSE
            % it can only be applied to the case that Mi=S
            u2 = MMSE(N,Mi,S,K,H,sigma2);
            p2 = ones(1,K*S);
            [count_temp,capacity_temp] = coherence_receive(N,Mi,S,K,KK_NoScheduling,Tc,s,x,H,sigma2,w,u2,p2);
            count2=count2+count_temp;
            capacity2=capacity2+real(capacity_temp);
            
            % BD
            [u3, r3] = BD(N,Mi,S,K,H,sigma2);
            p3 = ones(1,K*S);
            ...
            % SINR
            % it can only be applied to the case that S=1
            [u5, p5] = SINR(N,Mi,S,K,H,sigma2);
