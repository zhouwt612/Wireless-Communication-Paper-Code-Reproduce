function [KK_new,round_num,cal_num] = MMSLNR_modified(MT,K_all,candidate_num,K,R,H,sigma2,FB_bits)
...
H_now=zeros(K*R,MT);
for i1=1:K,
    H_now((i1-1)*R+1:i1*R,:)=H((KK_temp(1,i1)-1)*R+1:KK_temp(1,i1)*R,:);
end
...
while over~=0,
    ...
    for i1=1:candidate_num,
        ...
        if scheduled==0,
            ...
            for i2=1:K,
                H_now((i2-1)*R+1:i2*R,:)=H((temp1(1,i2)-1)*R+1:temp1(1,i2)*R,:);
            end

            Hi=zeros(R,MT);
            Hk=zeros(R,MT);
            for i2=1:K,
                Hi=H_now((i2-1)*R+1:i2*R,:);
                C=(MT-MT*err_UpperBound)*Hi'*Hi+err_UpperBound*MT*eye(MT);

                D=R*sigma2*eye(MT);
                for i3=1:K,
                    if i3~=i2,
                        Hk=H_now((i3-1)*R+1:i3*R,:);
                        D=D+(MT-MT*err_UpperBound)*Hk'*Hk+err_UpperBound*MT*eye(MT);
                    end
                end
