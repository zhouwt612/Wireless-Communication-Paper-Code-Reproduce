function [u, ant_num] = SLNR_MultiAnt_modified(N,Mi,K,KK,H,sigma2,FB_bits)

ant_num=ones(1,K);

err_UpperBound=2^(-FB_bits/(N-1));
...
for i1=1:K,
    Hi=H_now(i1,:);
    C=(N-N*err_UpperBound)*Hi'*Hi+err_UpperBound*eye(N);

    D=sigma2*eye(N);
    for i2=1:K,
        if i2~=i1,
            Hk=H_now(i2,:);
            D=D+(N-N*err_UpperBound)*Hk'*Hk+err_UpperBound*eye(N);
        end
    end
...
while 0==0,
    ...
    for i1=1:K,
        ...
        err_UpperBound=2^(-ant_num(1,i1)*FB_bits/(N-1));
        C=(N-N*err_UpperBound)*Hi'*Hi+err_UpperBound*ant_num(1,i1)*eye(N);
        ...
        for i2=1:K,
            if i2~=i1,
                ...
                err_UpperBound=2^(-ant_num(1,i2)*FB_bits/(N-1));
                D=D+(N-N*err_UpperBound)*Hk'*Hk+err_UpperBound*ant_num(1,i2)*eye(N);
            end
        end
